function TotalAdvantageValueStrategy2(handles,hObject)
playableHand=[];
load('hand1');
load('hand2');
load('data');
...
if ~OneEyedJacks && ~TwoEyedJacks
    ...
    indexNormal=TotalAdvantageValue(handles,hObject,playableHand)
...
elseif OneEyedJacks && ~TwoEyedJacks
    ...
    if removableFound
        ...
        indexRemove=TotalAdvantageValue(handles,hObject,removableSpot)
...
if BlockCounter == 1
    for jCnt=1:length(playableHand)
        if strcmp(BlockOptions , playableHand(jCnt).loc)
...
if BlockCounter > 1
    for iCnt=1:BlockCounter
        for jCnt=1:length(playableHand)
            if strmcp(BlockOptions(iCnt) , playableHand(jCnt).loc)
                target=playableHand(jCnt).loc;
...
if WinCounter == 1
    for jCnt=1:length(playableHand)
        if strcmp(WinOptions , playableHand(jCnt).loc)
            target=playableHand(jCnt).loc;
            discard=playableHand(jCnt).num7;
        end
    end
...
if WinCounter > 1
    for iCnt=1:WinCounter
        for jCnt=1:length(playableHand)
            if strmcp(WinOptions(iCnt) , playableHand(jCnt).loc);
                target=playablehand(jCnt).loc;
            end
        end
    end
