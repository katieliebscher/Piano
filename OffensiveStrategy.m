function OffensiveStrategy(handles,hObject)
...
if ~OneEyedJacks && ~TwoEyedJacks
    ...
    [maxOff,index]=OffSubValFunction(playableHand);
...
elseif ~OneEyedJacks && TwoEyedJacks
    ...
    index=OffSubValueFunction(playableHand);
    for iCnt=1:14
...
elseif OneEyedJacks && ~TwoEyedJacks
    ...
    [maxOffNormal, indexNormal]=OffSubValFunction(playableHand);
    [maxOffRemove, indexRemove]=OffSubValFunction(removableSpot);
...
elseif TwoEyedJacks && OneEyedJacks
    ...
    [maxOffNormal, indexNormal]=max(playableHand);
    [maxOffRemove, indexRemove]=max(removableSpot);
