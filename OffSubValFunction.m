function [maxOff,index,playableHand]=OffSubValFunction(handles,hObject,playableHand)
...
for iCnt=1:length(playableHand);
        ...
        while ~ended && counter<5
            ...
            if R<=0 || R>=11 ||  C<=0 || C>=11
                ...
                if abs(difEnd(1))<0.01 && abs(difEnd(2))<0.01 && abs(difEnd(3))<0.01
                    ended=true;
                elseif abs(difAdd(1))<0.01 && abs(difAdd(2))<0.01 && abs(difAdd(3))<0.01
                    if multCounter==0
                        sumDown=sumDown+1;
                    else
                ...
                elseif abs(difBlack(1))<0.01 && abs(difBlack(2))<0.01 && abs(difBlack(3))<0.01
                    if multCounter==0
                        sumDown=sumDown+1;
                    else
                        sumDown=sumDown+multiplier^(multCounter);
                ...
                elseif abs(difYell(1))<0.01 && abs(difYell(2))<0.01 && abs(difYell(3))<0.01
        ...
        while ~ended && counter<5
            ...
            if R<=0 || R>=11 ||  C<=0 || C>=11
                ...
                difYell=yellow-currentColor;
                difBlack=black-currentColor;
                if abs(difEnd(1))<0.01 && abs(difEnd(2))<0.01 && abs(difEnd(3))<0.001
                    ended=true;
                elseif abs(difAdd(1))<0.01 && abs(difAdd(2))<0.01 && abs(difAdd(3))<0.01
                    if multCounter==0
        ...
        for iCnt=1:length(playableHand)
                if ~strcmp(playableHand(iCnt).loc,'R1C1') && ~strcmp(playableHand(iCnt).loc,'R1C10') && ~strcmp(playableHand(iCnt).loc,'R10C1') && ~strcmp(playableHand(iCnt).loc,'R10C10')
                    ...
                    playableHand(counter).num7=playableHand(iCnt).num7;
    ...
    %% Find best value
 [maxOff,index]=max([playableHand.offSubValues]);