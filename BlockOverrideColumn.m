function BlockOverrideColumn(handles,hObject)
...
sumBlue=0;
sumRed=0;
Block(1).BlockOptions=['0'] %This is the vector that holds the tagnames for every square
%a player can win! %%%%%%% YOU NEED THE FIRST TERM TO BE ZERO FOR THIS TO
%WORK

BlockCounter = 0; %Counter to increase the WinOption vector
                ...
                for j = 1+k:5+k
                    ...
                    if (handles.(tagBox).BackgroundColor ~= Red) 
                        if (handles.(tagBox).BackgroundColor ~= Black)
                            ...
                            Block(BlockCounter).BlockOptions = tagBox;
...
save('Blocker','Block','BlockCounter');